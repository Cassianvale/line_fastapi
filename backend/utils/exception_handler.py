...
from backend.utils.errors import BaseExceptionMixin
from backend.utils.log_control import logger
from backend.utils.response_control import response_api
...
async def _validation_exception_handler(e: RequestValidationError | ValidationError):
    ...
    return JSONResponse(status_code=422, content=await response_api.fail(code=422, msg=msg, data=data))
...
def register_exception(app: FastAPI):
    ...
    async def http_exception_handler(request: Request, exc: HTTPException):
        ...
        return JSONResponse(
            ...
            content=await response_api.fail(**content),
            headers=exc.headers,
        )
    ...
    async def pydantic_user_error_handler(request: Request, exc: PydanticUserError):
        ...
        return JSONResponse(
            status_code=500,
            content=await response_api.fail(code=exc.code, msg=CUSTOM_USAGE_ERROR_MESSAGES.get(exc.code)),
        )
    ...
    async def assertion_error_handler(request: Request, exc: AssertionError):
        ...
        return JSONResponse(
            status_code=500,
            content=await response_api.fail(
                code=500,
                msg=str(''.join(exc.args) if exc.args else exc.__doc__),
            )
            if settings.ENVIRONMENT == 'dev'
            else await response_api.fail(code=500, msg='Internal Server Error'),
        )
    ...
    async def all_exception_handler(request: Request, exc: Exception):
        ...
        if isinstance(exc, BaseExceptionMixin):
            ...
            return JSONResponse(
                status_code=500,
                content=await response_api.fail(code=500, msg=str(exc))
                if settings.ENVIRONMENT == 'dev'
                else await response_api.fail(code=500, msg='Internal Server Error'),
            )
    ...
    if settings.MIDDLEWARE_CORS:
        ...
        async def cors_status_code_500_exception_handler(request, exc):
            ...
            response = JSONResponse(
                ...
                if isinstance(exc, BaseExceptionMixin)
                else await response_api.fail(code=500, msg=str(exc))
                if settings.ENVIRONMENT == 'dev'
                else await response_api.fail(code=500, msg='Internal Server Error'),
